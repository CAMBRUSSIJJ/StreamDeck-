...
const required = [
  ...
  'js/core/local.js','js/core/protocol.js','js/core/store.js','js/core/profiles.js','js/core/integrations.js','js/ui/app-icons.js'
];
...
for (const id of ['onboarding-dialog','deck-diagnostic','deck-diagnostic-export','export-deck','import-deck','btn-app-icon']) {
  if (!html.includes(`id="${id}"`)) throw new Error(`elemento V1.1 ausente: ${id}`);
}
const app = await readFile(resolve(root, 'js/app.js'), 'utf8');
if (!app.includes("ONBOARDING_KEY")) throw new Error('onboarding V1.1 não inicializado');
if (!app.includes('buildDeckDiagnostic')) throw new Error('diagnóstico rápido ausente');
if (!app.includes('APP_ICON_OPTIONS')) throw new Error('biblioteca de ícones de apps V1.1 não inicializada');
...
if (!sw.includes('nexus-deck-v1.1.0')) throw new Error('cache do SW sem versão');
