...
for (const id of ['onboarding-dialog','deck-diagnostic','deck-diagnostic-export','export-deck','import-deck','btn-app-icon','layout-dialog','layout-open','layout-preset-grid']) {
  if (!html.includes(`id="${id}"`)) throw new Error(`elemento V1.2 ausente: ${id}`);
}
const app = await readFile(resolve(root, 'js/app.js'), 'utf8');
if (!app.includes("ONBOARDING_KEY")) throw new Error('onboarding V1.2 não inicializado');
if (!app.includes('buildDeckDiagnostic')) throw new Error('diagnóstico rápido ausente');
if (!app.includes('APP_ICON_OPTIONS')) throw new Error('biblioteca de ícones de apps V1.2 não inicializada');
...
if (!sw.includes('nexus-deck-v1.2.0')) throw new Error('cache do SW sem versão');
...
if (!layout.includes('Media Console')) throw new Error('presets profissionais ausentes');
