...
const required = [
  ...
  'js/core/profiles.js','js/core/integrations.js','js/core/companion-sync.js','js/ui/app-icons.js'
];
...
for (const id of ['onboarding-dialog','deck-diagnostic','export-deck','import-deck','btn-app-icon','layout-dialog','layout-open','layout-preset-grid','mobile-immersive-toggle','mobile-lock-toggle','mobile-swipe-toggle','mobile-longpress-toggle','mobile-scale','mobile-portrait-columns','mobile-landscape-columns','fullscreen-toggle','companion-sync-badge','companion-audio-status','spotify-now-playing']) {
  if (!html.includes(`id="${id}"`)) throw new Error(`elemento V1.5 ausente: ${id}`);
}
const app = await readFile(resolve(root, 'js/app.js'), 'utf8');
for (const symbol of ['ONBOARDING_KEY','buildDeckDiagnostic','APP_ICON_OPTIONS','bindMobileGestures','bindMobileLongPress','updateMobileEnvironment','toggleFullscreen','renderCompanionCenter']) {
  if (!app.includes(symbol)) throw new Error(`fundação V1.5 ausente: ${symbol}`);
}
const mobile = await readFile(resolve(root, 'js/core/mobile.js'), 'utf8');
if (!mobile.includes('columnsForViewport') || !mobile.includes('qualifiesAsSwipe')) throw new Error('Mobile Engine V1.5 incompleto');
...
if (!sw.includes('nexus-deck-v1.5.0')) throw new Error('cache do SW sem versão V1.5');
if (!sw.includes('/js/core/mobile.js')) throw new Error('Mobile Engine ausente do cache PWA');
if (!sw.includes('/js/core/companion-sync.js')) throw new Error('Companion Sync ausente do cache PWA');
