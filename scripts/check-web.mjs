...
const required = [
  ...
  'js/core/local.js','js/core/protocol.js','js/core/store.js','js/core/profiles.js','js/core/integrations.js'
];
...
for (const id of ['onboarding-dialog','deck-diagnostic','deck-diagnostic-export','export-deck','import-deck']) {
  if (!html.includes(`id="${id}"`)) throw new Error(`elemento V1.0 ausente: ${id}`);
}
const app = await readFile(resolve(root, 'js/app.js'), 'utf8');
if (!app.includes("ONBOARDING_KEY")) throw new Error('onboarding V1.0 não inicializado');
...
if (!sw.includes('nexus-deck-v1.0.0')) throw new Error('cache do SW sem versão');
