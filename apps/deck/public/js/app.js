...
const ONBOARDING_KEY = 'nexus.deck.onboarding.v1.8';
...
function buildDeckDiagnostic() {
  ...
  add('Interface Vercel', cloudConfig?.configured ? 'ok' : 'error', cloudConfig?.configured ? 'Nexus Relay disponível nesta origem' : 'Nexus Relay indisponível');
  add('Fallback local', localConfig?.configured ? 'ok' : 'ok', localConfig?.configured ? (localConfig.localUrl || 'Bridge local detectado') : 'Não necessário no modo Vercel');
  ...
  return {
    format:'nexus-deck-diagnostic', version:1, appVersion:'1.8.0', generatedAt:new Date().toISOString(),
...
function updateLiveWidgets(now = new Date()) {
  ...
  document.querySelectorAll('[data-live-latency]').forEach(el => {
    el.textContent = online && Number.isFinite(status?.latencyMs) ? `${Math.round(status.latencyMs)} ms · ${device?.transport === 'local' ? 'LAN' : 'Nexus Relay'}` : '';
  });
...
function renderDeviceState() {
  ...
  els.deviceStatusText.textContent = online ? (device.transport === 'local' ? `Local · ${Math.round(status?.latencyMs || 0)} ms` : 'Nexus Web · Online') : 'Offline';
...
function renderDeviceDialog() {
  const rows = state.devices.map(device => {
    ...
    row.querySelector('span').textContent = `${online ? '● Online' : '○ Offline'} · ${device.transport === 'local' ? 'LAN' : 'Nexus Relay'} · ${device.platform || 'Windows'}${online && Number.isFinite(status?.latencyMs) ? ` · ${Math.round(status.latencyMs)} ms` : ''}`;
  ...
  if (!rows.length) {
    ...
    empty.innerHTML = `<div class="device-meta"><strong>Nenhum computador pareado</strong><span>No Windows Bridge, configure a URL do Vercel, gere um código e digite-o aqui.</span></div>`;
    rows.push(empty);
  }
...
async function connectionForAction({ allowOfflineVisual = false } = {}) {
  ...
  if (device.transport !== 'local' && !connection?.channel?.joined) { toast('O Nexus Relay está reconectando.', 'error'); return null; }
    ...
    try {
      ...
      if (message.type === 'status') {
        ...
        if (message.body?.state) {
          const status = normalizeCompanionStatus(message.body.state, { seenAt:Date.now(), transport:'cloud' });
          deviceStatuses.set(device.id, status);
          if (device.id === activeDevice()?.id) applySmartProfile(status);
          renderDeviceState();
          if (els.deviceDialog.open) renderDeviceDialog();
...
async function pairDevice(code) {
  ...
  if (!cloudConfig?.configured) throw new Error('O Nexus Relay desta publicação não está disponível. Atualize o projeto no Vercel.');
...
async function loadLocalConfig() {
  try {
    ...
    if (els.localStatus) { els.localStatus.textContent = 'Não necessário'; els.localStatus.style.color = '#9aa4b4'; }
    if (els.localUrl) els.localUrl.textContent = location.host;
  }
}
...
async function loadCloudConfig() {
  try {
    ...
    if (!response.ok || !json.configured || !json.relayUrl) throw new Error('Nexus Relay não disponível');
    cloudConfig = json;
    els.cloudStatus.textContent = 'Vercel · Online';
    els.cloudStatus.style.color = '#42d98b';
    await connectAllDevices();
  } catch {
    cloudConfig = { configured:false };
    els.cloudStatus.textContent = 'Indisponível';
